...
function createWindow(): void {
  ...
  ipcMain.handle('get-config', async () => {
    ...
    let config = yaml.load(configStr) as ConfigSchema;

    if (config.githubToken.includes('$')) {
      let envVar = process.env[config.githubToken.substring(1)];
      if (envVar !== undefined) {
        config.githubToken = envVar;
      }
    }
